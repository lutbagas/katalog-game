...
export function GameCard({game}: {game : Game}) {
  ...
  <div className="bg-gradient-to-br from-purple-700/60 via-violet-800/50 to-blue-800/60 p-4 rounded-xl">
    <div className="mb-3">
      ...
      alt="" width={500} height={500} loading="lazy" quality={90} className={game.background_image? "rounded-lg object-cover": "rounded-lg bg-gray-800 p-1.5"}></Image>
    </div>
    <div className="space-y-1 pb-4.5">
      ...
      <p className="text-center">Genre: {game.genres[1]?.name}</p>
    </div>
    <div className="text-center">
      <Link href={`/game/${game.id}`} className="bg-sky-600/60 p-2.5 rounded-xl hover:bg-sky-500/90" >Detail</Link>
    </div>
  </div>
  )
}